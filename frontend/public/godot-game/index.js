...
Engine.prototype.gameLoop = function() {
    ...
    // Check for wallet address from parent window or local
    const walletAddress = window.walletAddress || this.walletAddress;
    if (walletAddress) {
        ...
        this.ctx.font = '16px Arial';
        this.ctx.fillText('❌ Waiting for Wallet...', this.canvas.width / 2, 200);
        
        this.ctx.fillStyle = '#ffffff';
        this.ctx.font = '14px Arial';
        this.ctx.fillText('Requesting wallet info from parent...', this.canvas.width / 2, 230);
        
        // Request wallet info from parent
        if (window.parent && window.parent !== window) {
            parent.postMessage({ type: 'REQUEST_WALLET_INFO' }, '*');
        }
    }
...
// Listen for wallet info from parent
window.addEventListener('message', (event) => {
    if (event.data.type === 'WALLET_INFO') {
        window.walletAddress = event.data.address;
        window.ethereum = event.data.ethereum;
    }
});